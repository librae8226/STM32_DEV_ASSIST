...
//SPI 读数据  
//从7846/7843/XPT2046/UH7843/UH7846 读取 adc 值      
...
{     
	...
	delay_us(6);//ADS7846的转换时间最长为 6us 
	TCLK=1;//给1 个时钟，清除 BUSY          
	TCLK=0;     
...
//2 次读取ADS7846,连续读取 2 次有效的 AD 值,且这两次的偏差不能超过 
...
{          
 ...
 { 
  ...
  Pen_Point.xoff=(240-Pen_Point.xfac*(pos_temp[1][0]+pos_temp[0][0]))/2;// 得 到
xoff 
         
    
  Pen_Point.yfac=(float)280/(pos_temp[2][1]-pos_temp[0][1]);//得到yfac 
    
  Pen_Point.yoff=(320-Pen_Point.yfac*(pos_temp[2][1]+pos_temp[0][1]))/2;// 得 到
yoff   
     POINT_COLOR=BLUE; 
     LCD_Clear(WHITE);//清屏 
...
{                  
  ...
  RCC->APB2ENR|=0x01;    //使能 io复用时钟       
    AFIO->EXTICR[0]|=0X0020; //EXTI13 映射到PC1    
  EXTI->IMR|=1<<1;        //开启line1 上的中断 
  EXTI->EMR|=1<<1;        //不屏蔽line1 上的事件 
